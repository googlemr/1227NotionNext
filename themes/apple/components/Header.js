  ...
  const scrollTrigger = useCallback(
    throttle(() => {
      requestAnimationFrame(() => {
        ...
        if (navTransparent) {
        nav?.classList?.add('apple-nav--transparent')
        nav?.classList?.remove('apple-nav--solid')
        } else {
          nav?.classList?.add('apple-nav--solid')
        nav?.classList?.remove('apple-nav--transparent')
        }
  ...
  return (
    <div id='top-nav'>
      ...
      <div
        id='sticky-nav'
        className={
          'apple-nav flex justify-center top-0 fixed w-full z-30 transform transition-all duration-200'
        }>
        <div className='w-full max-w-6xl flex justify-between items-center px-4 py-3'>
          {/* 左侧功能 */}
